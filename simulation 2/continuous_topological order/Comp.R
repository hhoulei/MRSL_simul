...
Comp <- function(x,N,D,each_g,gl,gu,prob,dlB,duB){
  ...
  fdata <- DataGeneration(N,D,each_g,gl,gu,prob,dlB,duB)
  R <- fdata$D_beta
  ...
  if ('try-error' %in% class(x.inv1)){
    res12 <- rep(NA,11)
    time12 <- NA
    order_met12 <- rep(NA,4)
  } else{
    result1 <- x.inv1
    time12 <- as.numeric(difftime(Sys.time(),ptm,units = "secs"))
    res12 <- unlist(calc_metrics(X=result1$amatt, X_true=result1$R1))
    order_met12 <- c(result1$SpearF,result1$kenDcor_res)
  }
  rm(x.inv1)
  ...
  if ('try-error' %in% class(x.inv1)){
    res13 <- rep(NA,11)
    time13 <- NA
    order_met13 <- rep(NA,4)
  } else{
    result1 <- x.inv1
    time13 <- as.numeric(difftime(Sys.time(),ptm,units = "secs"))
    res13 <- unlist(calc_metrics(X=result1$amatt, X_true=result1$R1))
    order_met13 <- c(result1$SpearF,result1$kenDcor_res)
  }
  rm(x.inv1)
  ...
  ret <- list(res11=res11,res12=res12,res13=res13,
              ...
              res4=res4,res51=res51,res52=res52,res53=res53,
              r2=rr,Fsta=ff,
              time11=time11, time12=time12, time13=time13,
              time2=time2,time3=time3,time21=time21,time31=time31,
              time4=time4,time51=time51,time52=time52,time53=time53,
              order_met11=order_met11,
              order_met12=order_met12,
              order_met13=order_met13)
  return(ret)
