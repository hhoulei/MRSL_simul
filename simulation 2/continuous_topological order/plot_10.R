...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D10/D_10_dlB_0_duB_0.25_",i,".csv"),header =F)
...
dt21 <- data.frame(
  ...
  eachg=rep(dt1$each_g,11),
  p=paste0("Prob=",rep(dt1$prob,11))
)


dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D10/D_10_dlB_0.25_duB_0.5_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D10/D_10_dlB_0.5_duB_0.75_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D10/D_10_dlB_0.75_duB_1_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dttime <- cbind(rep(c("0-0.25","0.25-0.5","0.5-0.75","0.75-1"),each=18),dttime)
dttime1 <- dttime[,c(1:3,145:155)]
colnames(dttime1) <- c('edge effect','each g','prob',
                       ...
                       'BIMMER','cGAUGE.ivw','cGAUGE.egger','cGAUGE.mrpresso')
write.csv(dttime1,"D10_computing_time.csv")

dttime2 <- dttime[,c(1:3,125:144)]
colnames(dttime2)[1:3] <- c('edge effect','each g','prob')
write.csv(dttime2,"D10_r2_F.csv")


dt2 <- as.data.frame(rbind(dt21,dt22,dt23,dt24))
dt2$edge_effect <- c(rep(c("edges_effect 0-0.25","edges_effect 0.25-0.5",
                           "edges_effect 0.5-0.75","edges_effect 0.75-1"),each=198))
...
dt2$timee <- ifelse(dt2$timee>20,20,dt2$timee)
pyt <- ggplot(data=dt2, aes(x=eachg, y=timee,colour=method,group=method)) +
  ...
  #scale_colour_brewer(palette = "Paired") +
  facet_grid( vars(p), vars(edge_effect),scales="free_x")+
  ggtitle("Computing time with 10 continuous nodes") +
...
pdf(file="simulation_D10_time.pdf",height = 8,width = 12)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2221, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  ggtitle("Precision-Recall with 10 continuous nodes when edges' effect between 0-0.25") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D10_PRcurve_0_0.25.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2222, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 10 continuous nodes when edges' effect between 0.25-0.5 ") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D10_PRcurve_0.25_0.5.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2223, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 10 continuous nodes when edges' effect between 0.5-0.75") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D10_PRcurve_0.5_0.75.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2224, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 10 continuous nodes when edges' effect between 0.75-1") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D10_PRcurve_0.75_1.pdf",height = 6,width = 13)  
pyt 
dev.off()
