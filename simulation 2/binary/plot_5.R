...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D5/D_5_dlB_0_duB_0.41_",i,".csv"),header =F)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D5/D_5_dlB_0.41_duB_0.69_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D5/D_5_dlB_0.69_duB_0.92_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D5/D_5_dlB_0.92_duB_1.1_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dttime <- cbind(rep(c("OR 1-1.5","OR 1.5-2","OR 2-2.5","OR 2.5-3"),each=18),dttime)
colnames(dttime)
...
write.csv(dttime1,"D5_computing_time.csv")
...
pyt <- ggplot(data=dt2, aes(x=eachg, y=F1_score,colour=method,group=method)) +
  ...
  ylab("F1 score") +
  ggtitle("F1 score with 5 binary nodes") +
  theme_bw() +
...
pdf(file="simulation_D5_F1score.pdf",height = 8,width = 12)  
pyt 
dev.off()



dt2$timee <- ifelse(dt2$timee>1.5,1.5,dt2$timee)
pyt <- ggplot(data=dt2, aes(x=eachg, y=timee,colour=method,group=method)) +
  ...
  #scale_colour_brewer(palette = "Paired") +
  facet_grid( vars(p), vars(edge_effect),scales="free_x")+
  ggtitle("Computing time with 5 binary nodes") +
...
pdf(file="simulation_D5_time.pdf",height = 8,width = 12)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2221, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  ggtitle("Precision-Recall with 5 binary nodes when OR between 1-1.5") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D5_PRcurve_1_1.5.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2222, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 5 binary nodes when OR between 1.5-2") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D5_PRcurve_1.5_2.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2223, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 5 binary nodes when OR 2-2.5") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D5_PRcurve_2_2.5.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2224, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 5 binary nodes when OR between 2.5-3") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D5_PRcurve_2.5_3.pdf",height = 6,width = 13)  
pyt 
dev.off()
