...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D15/D_15_dlB_0_duB_0.41_",i,".csv"),header =F)
...
dt21 <- data.frame(
  ...
  eachg=rep(dt1$each_g,5),
  p=paste0("Prob=",rep(dt1$prob,5))
)


dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D15/D_15_dlB_0.41_duB_0.69_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D15/D_15_dlB_0.69_duB_0.92_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:18){
  dt1_once <- read.csv(paste0("D15/D_15_dlB_0.92_duB_1.1_",i,".csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dttime <- cbind(c(rep("OR 1-1.5",18),rep(c("OR 1.5-2","OR 2-2.5","OR 2.5-3"),each=18)),dttime)
colnames(dttime)
dttime1 <- dttime[,c(1:3,59:63)]
...
write.csv(dttime1,"D15_computing_time.csv")


dt2 <- as.data.frame(rbind(dt21,dt22,dt23,dt24))
dt2$edge_effect <- c(rep("OR 1-1.5",90),
                     rep(c("OR 1.5-2",
                           "OR 2-2.5","OR 2.5-3"),each=90))

shapes <- c(17,17,rep(8,3))
coloo <- c(
           "#FDAE6B","#F16913",
           
           "#FC8D62","#E7298A","#E31A1C"
           )
dt2[c(282,300,318),2:7] <- dt2[c(282,300,318)-3,2:7]
pyt <- ggplot(data=dt2, aes(x=eachg, y=F1_score,colour=method,group=method)) +
  ...
  ylab("F1 score") +
  ggtitle("F1 score with 15 binary nodes") +
  theme_bw() +
...
pdf(file="simulation_D15_F1score.pdf",height = 8,width = 12)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2221, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  ggtitle("Precision-Recall with 15 binary nodes when OR between 1-1.5") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D15_PRcurve_1_1.5-1.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2222, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 15 binary nodes when OR between 1.5-2") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D15_PRcurve_1.5_2.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2223, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 15 binary nodes when OR 2-2.5") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D15_PRcurve_2_2.5.pdf",height = 6,width = 13)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2224, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  #facet_wrap(~eachg_name,nrow=2) +
  facet_grid(vars(p), vars(eachg_name),scales="free_x")+
  ggtitle("Precision-Recall with 15 binary nodes when OR between 2.5-3") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5))+
...
pdf(file="simulation_D15_PRcurve_2.5_3.pdf",height = 6,width = 13)  
pyt 
dev.off()
