...
me <- c('TP','FP','FN','TN','precision','recall','F1','acc','TPR','TNR','FPR','SHD')
...
for(i in 1:length(prob)){
  tt1 <- NULL
  load(paste0('Magic__prob_corp_',prob[i],'_addorder.Rdata'))
  ...
  res_all4 <- res_all[,paste0('SHD_',method_name)]
  res_SHD <- rbind(res_SHD,
                      apply(res_all4,2,function(x) mean(x,na.rm = T)))
...
res_step1_SHD <- res_step1_all[,grep(pattern = 'SHD',colnames(res_step1_all))]
...
write.csv(res_step1_spearF,'res_step1_spearF.csv')
write.csv(res_step1_kenDcor,'res_step1_kenDcor.csv')
...
### plot
library(ggplot2)
res_SHD <- as.data.frame(res_SHD)
res_precision <- as.data.frame(res_precision)
res_recall <- as.data.frame(res_recall)
datares <- data.frame(F1score=c(res_SHD$SHD_ivw,
                                res_SHD$SHD_egger,
                                res_SHD$`SHD_weighted median`,
                                res_SHD$`SHD_Mode based`,
                                res_SHD$SHD_MR.Mix,
                                res_SHD$SHD_MR.RAPS,
                                res_SHD$SHD_CDcML.DP.S,
                                res_SHD$SHD_CD.Egger,
                                res_SHD$SHD_MRPC_sigSNP,
                                res_SHD$`SHD_HC+prior_sigSNP`,
                                res_SHD$`SHD_HC+prior_score`,
                                res_SHD$SHD_MRPC_score,
                                res_SHD$SHD_BIMMER,
                                res_SHD$SHD_cGAUGE.ivw,
                                res_SHD$SHD_CD.Egger,
                                res_SHD$SHD_cGAUGE.mrpresso),
...
p1 <- ggplot(data=datares,mapping=aes(x=prob,y=F1score,fill=method))+
  ...
  theme_bw() +
  ggtitle("SHD (correlated pleiotropy)") +
...
pdf("SHD (correlated pleiotropy).pdf",width = 20,height = 7)
p1
dev.off()
