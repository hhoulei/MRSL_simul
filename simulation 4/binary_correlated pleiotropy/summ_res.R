...
for(i in 1:length(prob)){
  tt1 <- NULL
  load(paste0('Asia__prob_corp_',prob[i],'_addorder.Rdata'))
...
p1 <- ggplot(data=datares,mapping=aes(x=prob,y=F1score,fill=method))+
  ...
  theme_bw() +
  ggtitle("F1 score (correlated pleiotropy)") +
...
pdf("F1 score (correlated pleiotropy).pdf",width = 20,height = 7)
p1
dev.off()


shapes <- c(15,15,15,17,17,5,5,rep(8,8))
...
p1 <- ggplot(data=datares,mapping=aes(x=recall,y=precision,
  ...
  ggtitle("Precision-Recall (correlated pleiotropy)") +
  xlab("Recall") +
  ylab("Presion") +
  xlim(0,1)+
  ylim(0,1)+
...
pdf("Precision-Recall (correlated pleiotropy).pdf",width = 20,height = 5)
p1
dev.off()
