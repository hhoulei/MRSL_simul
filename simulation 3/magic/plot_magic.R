...
for(i in 1:length(gg)){
  dt1_once <- read.csv(paste0("magic_each_g_",gg[i],"_dlB_0_duB_0.25.csv"),header =F)
...
dt1 <- NULL
for(i in 1:length(gg)){
  dt1_once <- read.csv(paste0("magic_each_g_",gg[i],"_dlB_0.25_duB_0.5.csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:length(gg)){
  dt1_once <- read.csv(paste0("magic_each_g_",gg[i],"_dlB_0.5_duB_0.75.csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
dt1 <- NULL
for(i in 1:length(gg)){
  dt1_once <- read.csv(paste0("magic_each_g_",gg[i],"_dlB_0.75_duB_1.csv"),header =F)
  dt1_once <- apply(dt1_once,2,function(x) mean(x,na.rm=T))
  dt1 <- rbind(dt1,dt1_once)
}
dt1 <- as.data.frame(dt1)
...
write.csv(dttime1,"MAGIC_computing_time.csv")

dttime2 <- dttime[,c(1:2,124:137)]
colnames(dttime2)[1:2] <- c('edge effect','each g')
write.csv(dttime2,"MAGIC_r2_F.csv")



dt2 <- as.data.frame(rbind(dt21,dt22,dt23,dt24))
dt2$edge_effect <- c(rep("edges_effect 0-0.25",66),rep(c("edges_effect 0.25-0.5",
                                                         "edges_effect 0.5-0.75",
                         "edges_effect 0.75-1"),each=66))
...
pyt <- ggplot(data=dt2, aes(x=eachg, y=F1_score,colour=method,group=method)) +
  ...
  ggtitle("F1 score with MAGIC graph") +
...
pdf(file="magic_F1score.pdf",height = 6,width = 7.5)  
pyt 
dev.off()


dt2$timee <- ifelse(dt2$timee>6,6,dt2$timee)
pyt <- ggplot(data=dt2, aes(x=eachg, y=timee,colour=method,group=method)) +
  ...
  #scale_colour_brewer(palette = "Paired") +
  facet_wrap(~edge_effect,nrow=2,scales = "free_x") +
  ggtitle("Computing time with MAGIC graph") +
  xlab("The number of SNPs") +
  ylab("Time") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.5,vjust = 0.5)) 
...
pdf(file="magic_time.pdf",height = 6,width = 7.5)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2221, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  ggtitle("Precision-Recall with MAGIC graph when edges' effect between 0-0.25") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
...
pdf(file="magic_PRcurve_0_0.25.pdf",height = 6,width = 10)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2222, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  facet_wrap(~eachg_name,nrow=2) +
  #facet_grid(edge_effect~eachg_name) +
  ggtitle("Precision-Recall with MAGIC graph when edges' effect between 0.25-0.5") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.2,vjust = 0.5))+
...
pdf(file="magic_PRcurve_0.25_0.5.pdf",height = 6,width = 10)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2223, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  facet_wrap(~eachg_name,nrow=2) +
  #facet_grid(edge_effect~eachg_name) +
  ggtitle("Precision-Recall with MAGIC graph when edges' effect between 0.5-0.75") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.2,vjust = 0.5))+
...
pdf(file="magic_PRcurve_0.5_0.75.pdf",height =6,width = 10)  
pyt 
dev.off()
...
pyt <- ggplot(data=dt2224, aes(x=Recall, y=Presion,colour=method,group=method)) +
  ...
  facet_wrap(~eachg_name,nrow=2) +
  #facet_grid(edge_effect~eachg_name) +
  ggtitle("Precision-Recall with MAGIC graph when edges' effect between 0.75-1") +
  xlab("Recall") +
  ylab("Presion") +
  theme_bw() +
  theme(plot.title = element_text(size = 15,hjust = 0.2,vjust = 0.5))+
...
pdf(file="magic_PRcurve_0.75_1.pdf",height = 6,width = 10)  
pyt 
dev.off()
